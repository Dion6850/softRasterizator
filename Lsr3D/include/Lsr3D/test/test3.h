...
// draw a triangle
void draw_triangle(const Eigen::Vector4f& v0_, const Eigen::Vector4f& v1_, const Eigen::Vector4f& v2_
) {
    Eigen::Vector4f v0 = v0_;
    Eigen::Vector4f v1 = v1_;
    Eigen::Vector4f v2 = v2_;
    // 三角形绕轴旋转
...
void task() {
    ...
    for (auto& triangle_ : triangles) {
        ...
        draw_triangle(vertices.at(triangle.v0).position, vertices.at(triangle.v1).position, vertices.at(triangle.v2).position);
    }
}
...
void Init() {
    render::instance.modelLoader.loadModel("assets/model/bunny/bunny.obj");
}
